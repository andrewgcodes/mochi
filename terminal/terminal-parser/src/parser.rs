...
pub struct Parser {
    ...
    private_marker: bool,
    /// The actual private marker byte ('?', '>', '<', '=')
    private_marker_char: Option<u8>,
    /// OSC/DCS string data
    osc_data: Vec<u8>,
    /// DCS parameters
    dcs_params: Vec<u8>,
...
impl Parser {
        ...
        F: FnMut(Action),
    {
        match byte {
            b'?' | b'>' | b'<' | b'=' => {
                ...
                self.private_marker_char = Some(byte);
        ...
        F: FnMut(Action),
    {
        ...
        let action = CsiAction {
            ...
            private_marker: self.private_marker_char,
        };
