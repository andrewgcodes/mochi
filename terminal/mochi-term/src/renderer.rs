...
use crate::split_pane::{Rect, SplitDirection};
...
impl Renderer {
    ...
    /// Render multiple panes with dividers between them
    pub fn render_panes(
        &mut self,
        panes: &[PaneRenderInfo<'_>],
        dividers: &[(Rect, SplitDirection)],
        tab_bar_height: u32,
        tabs: &[TabInfo<'_>],
        active_tab: usize,
    ) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for pane in panes {
            ...
            for row in 0..rows {
                ...
                for col in 0..cols.min(line.cols()) {
                    ...
                    // Position relative to pane rect
            ...
            if scrollback_len > 0 {
                let pane_scrollbar_x = pane_rect.x + pane_rect.width.saturating_sub(12);
            ...
            // Draw focus indicator (subtle border) for focused pane when there are multiple panes
        ...
        for (rect, _direction) in dividers {
            Self::fill_rect_static(
                &mut buffer,
                rect.x as i32,
                rect.y as i32,
    ...
    /// Draw a scrollbar within a specific pane rectangle
    ...
    fn draw_scrollbar_in_rect(
        ...
        let scrollbar_width: i32 = 12;
        let scrollbar_height = pane_rect.height as i32;
        let y_off = pane_rect.y as i32;
