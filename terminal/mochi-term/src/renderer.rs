...
pub struct TabInfo<'a> {
    pub title: &'a str,
    pub rename_cursor: Option<usize>,
}
...
impl Renderer {
    ...
    fn draw_tab_bar_static(
        ...
        for (i, tab) in tabs.iter().enumerate() {
            ...
            if let Some(cursor_pos) = tab.rename_cursor {
                let input_bg = Self::blend_color(bg_color, (255, 255, 255), 0.1);
                Self::fill_rect_static(
                    buffer,
                    tab_x + tab_padding as i32 - 2,
                    text_y,
                    max_text_width + 4,
                ...
                let cursor_x = text_x + (cursor_pos as f32 * cell_size.width) as i32;
                let cursor_color = Self::blend_color(fg_color, (100, 149, 237), 0.5);
                Self::fill_rect_static(
                    buffer,
                    cursor_x,
                    text_y + 1,
