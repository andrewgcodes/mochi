...
impl Renderer {
    ...
    /// Render multiple panes with dividers
    ...
    pub fn render_panes(
        ...
        for pane_info in panes {
            ...
            // Pane pixel origin (rect is in content-area coords, add tab_bar_height for y)
            let pane_x0 = rect.x as i32;
            let pane_y0 = rect.y as i32 + tab_bar_height as i32;
            ...
            for row in 0..rows {
                ...
                for col in 0..cols.min(line.cols()) {
                    ...
                    let x = pane_x0 + (col as f32 * cell_width_px) as i32;
                    let y = pane_y0 + (row as f32 * cell_height_px) as i32;
            ...
            // Draw active pane border (subtle highlight)
            if pane_info.is_active && panes.len() > 1 {
                let border_color = (80, 130, 200); // Blue-ish highlight
                Self::draw_rect_outline_static(
                    &mut buffer,
                    pane_x0,
                    pane_y0,
        ...
        if !dividers.is_empty() {
            let divider_color = (80, 80, 80);
            for divider in dividers {
                let dx = divider.rect.x as i32;
                let dy = divider.rect.y as i32 + tab_bar_height as i32;
                let dw = divider.rect.width as i32;
                let dh = divider.rect.height as i32;
                Self::fill_rect_static(&mut buffer, dx, dy, dw, dh, divider_color, width, height);
            }
        }
    ...
    /// Draw a scrollbar within a pane's rect
    #[allow(clippy::too_many_arguments)]
    fn draw_pane_scrollbar(
        ...
        let scrollbar_x = pane_x + pane_w - scrollbar_width;
        let scrollbar_height = pane_h;
