...
use crate::pane::{DividerInfo, PaneRect, SplitDirection, DIVIDER_WIDTH};
...
impl Renderer {
    ...
    pub fn render_panes(
        ...
        for pane_info in panes {
            ...
            // Draw scrollbar for this pane if there's scrollback content
            if scrollback_len > 0 {
        ...
        if panes.len() > 1 {
            let active_border_color = Self::blend_color(fg_color, (100, 149, 237), 0.6);
            for pane_info in panes {
                if pane_info.is_active {
                    ...
                    Self::fill_rect_static(
                        &mut buffer,
                        r.x as i32,
                        (r.y + r.h).saturating_sub(2) as i32,
                        r.w as i32,
                        2,
                        active_border_color,
                        width,
                    ...
                    Self::fill_rect_static(
                        &mut buffer,
                        (r.x + r.w).saturating_sub(2) as i32,
                        r.y as i32,
                        2,
                        r.h as i32,
                        active_border_color,
    ...
    fn draw_pane_scrollbar_static(
        ...
        let scrollbar_x = (pane_rect.x + pane_rect.w) as i32 - scrollbar_width;
        let scrollbar_height = pane_rect.h as i32;
