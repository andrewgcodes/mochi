...
impl PaneNode {
    fn new_leaf(id: PaneId, terminal: Terminal, child: Child) -> Self {
        PaneNode::Leaf {
            id, terminal, child, scroll_offset: 0, title: String::from("Terminal"),
        }
    }
    ...
    fn find_leaf_mut(
        &mut self, pane_id: PaneId,
    ) -> Option<(&mut Terminal, &mut Child, &mut usize, &mut String)> {
        match self {
            PaneNode::Leaf { id, terminal, child, scroll_offset, title }
    ...
    fn find_leaf_ref(&self, pane_id: PaneId) -> Option<(&Terminal, &Child, usize)> {
        match self {
            PaneNode::Leaf { id, terminal, child, scroll_offset, .. }
        ...
        match direction {
            SplitDirection::Vertical => {
                ...
                (
                    ...
                    PaneRect { x: rect.x + first_w + DIVIDER_WIDTH, y: rect.y, width: second_w, height: rect.height },
                )
            }
            SplitDirection::Horizontal => {
                ...
                (
                    ...
                    PaneRect { x: rect.x, y: rect.y + first_h + DIVIDER_WIDTH, width: rect.width, height: second_h },
                )
            }
        }
    }
    ...
    fn divider_rect(rect: PaneRect, direction: SplitDirection, ratio: f32) -> PaneRect {
        match direction {
            SplitDirection::Vertical => {
                let usable = rect.width.saturating_sub(DIVIDER_WIDTH);
                let first_w = (usable as f32 * ratio) as u32;
                PaneRect { x: rect.x + first_w, y: rect.y, width: DIVIDER_WIDTH, height: rect.height }
            }
            SplitDirection::Horizontal => {
                let usable = rect.height.saturating_sub(DIVIDER_WIDTH);
                let first_h = (usable as f32 * ratio) as u32;
                PaneRect { x: rect.x, y: rect.y + first_h, width: rect.width, height: DIVIDER_WIDTH }
            }
        }
    }
    ...
    fn resize_pane(&mut self, pane_id: PaneId, cols: usize, rows: usize) {
        match self {
            PaneNode::Leaf { id, terminal, child, .. } if *id == pane_id => {
    ...
    where F: FnMut(PaneId, &mut Terminal, &mut Child, &mut usize, &mut String),
    {
        match self {
            PaneNode::Leaf { id, terminal, child, scroll_offset, title } => {
    ...
    fn split_pane(
        self, target_id: PaneId, direction: SplitDirection,
        new_id: PaneId, new_terminal: Terminal, new_child: Child,
    ) -> (Self, bool) {
        match self {
            PaneNode::Leaf { id, terminal, child, scroll_offset, title } => {
                if id == target_id {
                    let old_leaf = PaneNode::Leaf { id, terminal, child, scroll_offset, title };
                    let new_leaf = PaneNode::new_leaf(new_id, new_terminal, new_child);
                    (PaneNode::Split {
                        direction, ratio: 0.5,
                        first: Box::new(old_leaf), second: Box::new(new_leaf),
                    }, true)
                } else {
                    (PaneNode::Leaf { id, terminal, child, scroll_offset, title }, false)
                }
            }
            PaneNode::Split { direction: dir, ratio, first, second } => {
                if first.contains_pane(target_id) {
                    ...
                    (PaneNode::Split { direction: dir, ratio, first: Box::new(new_first), second }, found)
                } else {
                    let (new_second, found) = (*second).split_pane(
                        target_id, direction, new_id, new_terminal, new_child,
                    );
                    (PaneNode::Split {
                        direction: dir, ratio,
                        first, second: Box::new(new_second),
                    }, found)
                }
            }
        }
    ...
    fn remove_pane(self, target_id: PaneId) -> Option<Self> {
        match self {
            PaneNode::Leaf { id, terminal, child, scroll_offset, title } => {
                if id == target_id { None }
                else { Some(PaneNode::Leaf { id, terminal, child, scroll_offset, title }) }
            }
            PaneNode::Split { direction, ratio, first, second } => {
                ...
                if in_first {
                    match (*first).remove_pane(target_id) {
                        Some(new_first) => Some(PaneNode::Split {
                            direction, ratio, first: Box::new(new_first), second,
                        }),
                        None => Some(*second),
                    }
                } else {
                    match (*second).remove_pane(target_id) {
                        Some(new_second) => Some(PaneNode::Split {
                            direction, ratio, first, second: Box::new(new_second),
                        }),
                        None => Some(*first),
                    }
                }
    ...
    fn dead_pane_ids(&self) -> Vec<PaneId> {
        match self {
            PaneNode::Leaf { id, child, .. } => {
                if child.is_running() { vec![] } else { vec![*id] }
            }
            PaneNode::Split { first, second, .. } => {
...
impl App {
    ...
    fn content_rect(&self) -> PaneRect {
        let (w, h) = if let Some(r) = &self.renderer { (r.current_width(), r.current_height()) } else { (0, 0) };
        PaneRect { x: 0, y: self.tab_bar_height, width: w, height: h.saturating_sub(self.tab_bar_height) }
    }
    ...
    fn handle_tab_bar_click(&mut self, x: f64) {
        ...
        if click_x < tabs_end {
            ...
            if tab_index < self.tabs.len() {
                ...
                } else { self.switch_to_tab(tab_index); }
            }
        }
    }
    ...
    fn split_active_pane(&mut self, direction: SplitDirection) {
        ...
        let root = match tab.root.take() { Some(r) => r, None => return };
        let layout = root.compute_layout(content);
        let target_rect = layout.iter().find(|(pid, _)| *pid == target_id).map(|(_, r)| *r);
        let target_rect = match target_rect {
            Some(r) => r,
            None => { tab.root = Some(root); return; }
        };
        ...
        let new_child = match Child::spawn_shell(WindowSize::new(cols as u16, rows as u16)) {
            Ok(c) => { let _ = c.set_nonblocking(true); c }
            Err(e) => { log::error!("Failed to spawn shell for new pane: {}", e); tab.root = Some(root); return; }
        };
        ...
        if ok {
            ...
            self.needs_redraw = true;
            log::info!("Split pane {} {:?}, new pane {}", target_id, direction, new_id);
        }
    }
    ...
    fn close_active_pane(&mut self) {
        ...
        if root.pane_count() <= 1 {
            tab.root = Some(root);
            if !self.close_current_tab() { self.tabs.clear(); }
            return;
        }
        ...
        if let Some(ref r) = tab.root {
            ...
            let new_active = if old_idx < ids.len() { ids[old_idx] } else { *ids.last().unwrap_or(&ids[0]) };
    ...
    fn focus_next_pane(&mut self) {
        ...
        if ids.len() <= 1 { return; }
        let cur = ids.iter().position(|id| *id == tab.active_pane).unwrap_or(0);
    ...
    fn resize_all_panes_in_tab(&mut self, tab_idx: usize) {
        ...
        for (pid, rect) in layout {
            ...
            if cols > 0 && rows > 0 { root.resize_pane(pid, cols, rows); }
        }
    }
    ...
    fn handle_resize(&mut self, size: PhysicalSize<u32>) {
        ...
        let content = PaneRect {
            x: 0, y: tab_bar_height, width: size.width,
    ...
    fn handle_key_input(&mut self, event: &winit::event::KeyEvent) {
        ...
        if ctrl_shift {
            match &event.logical_key {
                ...
                Key::Character(c) if c == "]" => { self.focus_next_pane(); return; }
                Key::Character(c) if c == "[" => { self.focus_prev_pane(); return; }
                _ => {}
            }
        }
        ...
        if self.modifiers.super_key() && !self.modifiers.control_key() && !self.modifiers.alt_key() {
            match &event.logical_key {
                ...
                Key::Character(c) if c.to_lowercase() == "t" => { self.create_new_tab(); return; }
                Key::Character(c) if c.to_lowercase() == "w" => { self.close_active_pane(); return; }
                Key::Character(c) if c.to_lowercase() == "d" => { self.split_active_pane(SplitDirection::Vertical); return; }
                Key::Character(c) if c == "]" => { self.focus_next_pane(); return; }
                Key::Character(c) if c == "[" => { self.focus_prev_pane(); return; }
        ...
        let root = match tab.root.as_mut() { Some(r) => r, None => return };
        let (terminal, child, _, _) = match root.find_leaf_mut(active_pane) { Some(leaf) => leaf, None => return };
        ...
        if let Some(text) = event.text_with_all_modifiers() {
            if !text.is_empty() {
                ...
                if (1..=26).contains(&char_code) || char_code == 0x7F {
                    log::debug!("Sending control character from text_with_all_modifiers: {:?} (0x{:02x})", first_char, first_char as u8);
        ...
        if let Key::Character(c) = &event.logical_key {
            if let Some(ch) = c.chars().next() {
                ...
                if (1..=26).contains(&char_code) || char_code == 0x7F {
                    log::debug!("Sending control character from logical_key: {:?} (0x{:02x})", ch, ch as u8);
        ...
        if zoom_modifier {
            match &event.logical_key {
                ...
                Key::Named(NamedKey::ArrowUp) => { self.change_font_size(2.0); return; }
                Key::Named(NamedKey::ArrowDown) => { self.change_font_size(-2.0); return; }
                _ => {}
            }
        }
    ...
    fn pane_at_pixel(&self, px: f64, py: f64) -> Option<(PaneId, u16, u16)> {
        ...
        let Some(renderer) = &self.renderer else { return None };
        let cell_size = renderer.cell_size();
        let content = self.content_rect();
        let layout = root.compute_layout(content);
        for (pid, rect) in &layout {
    ...
    fn handle_mouse_input(&mut self, button: MouseButton, state: ElementState) {
        ...
        if button == MouseButton::Left {
            if state == ElementState::Pressed {
                if let Some(window) = &self.window {
                    ...
                    if self.mouse_pixel.0 >= window_width - scrollbar_width && self.mouse_pixel.1 >= self.tab_bar_height as f64 {
                        let tab = &self.tabs[self.active_tab];
                        let active_pane = tab.active_pane;
                        if let Some(root) = &tab.root {
                            if let Some((terminal, _, scroll_offset)) = root.find_leaf_ref(active_pane) {
        ...
        let active_pane = tab.active_pane;
        let root = match tab.root.as_mut() { Some(r) => r, None => return };
        let (terminal, child, scroll_offset, _) = match root.find_leaf_mut(active_pane) { Some(leaf) => leaf, None => return };
        ...
        if !modes.mouse_tracking_enabled() {
            if button == MouseButton::Left {
                ...
                if state == ElementState::Pressed {
                    terminal.screen_mut().selection_mut().start(Point::new(col, row), SelectionType::Normal);
                    self.needs_redraw = true;
                } else {
    ...
    fn handle_mouse_motion(&mut self, position: winit::dpi::PhysicalPosition<f64>) {
            ...
            if let Some(window) = &self.window {
                ...
                if let Some(root) = &mut tab.root {
                    if let Some((terminal, _, scroll_offset, _)) = root.find_leaf_mut(active_pane) {
                        let window_height = (window.inner_size().height as f64 - self.tab_bar_height as f64).max(1.0);
                        let scrollback_len = terminal.screen().scrollback().len();
                        ...
                        if scrollback_len > 0 && window_height > 0.0 {
                            ...
                            let thumb_height = ((visible_rows as f64 / total_lines as f64) * window_height).max(20.0);
        ...
        if !modes.mouse_tracking_enabled() && self.mouse_buttons[0] {
            ...
            terminal.screen_mut().selection_mut().update(Point::new(sel_col, sel_row));
            self.needs_redraw = true;
            return;
        }
    ...
    fn handle_mouse_scroll(&mut self, delta: MouseScrollDelta) {
        ...
        if modes.mouse_tracking_enabled() || modes.alternate_screen {
            ...
            let scrollback_len = terminal.screen().scrollback().len();
            if lines > 0 { *scroll_offset = (*scroll_offset + lines as usize).min(scrollback_len); }
            else { *scroll_offset = scroll_offset.saturating_sub((-lines) as usize); }
            self.needs_redraw = true;
        }
    }
    ...
    fn handle_copy(&mut self) {
        ...
        let active_pane = tab.active_pane;
        let Some(root) = &tab.root else { return };
        let Some((terminal, _, _)) = root.find_leaf_ref(active_pane) else { return };
        ...
        if let Err(e) = clipboard.set_text(&text) { log::warn!("Failed to copy to clipboard: {}", e); }
        else { log::debug!("Copied {} bytes to clipboard", text.len()); }
    }

    fn handle_paste(&mut self) {
        let Some(clipboard) = &mut self.clipboard else { log::warn!("Clipboard not available"); return; };
        if self.tabs.is_empty() { return; }
        let tab = &mut self.tabs[self.active_tab];
        let active_pane = tab.active_pane;
        let root = match tab.root.as_mut() { Some(r) => r, None => return };
        ...
        match clipboard.get_text() {
            Ok(text) => {
                if text.is_empty() { return; }
                let data = if terminal.screen().modes().bracketed_paste { encode_bracketed_paste(&text) } else { text.into_bytes() };
                if let Err(e) = child.write_all(&data) { log::warn!("Failed to write paste data to PTY: {}", e); }
                else { log::debug!("Pasted {} bytes", data.len()); }
            }
            Err(e) => { log::warn!("Failed to get clipboard text: {}", e); }
        }
    }
    ...
    fn handle_new_window(&mut self) {
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            match std::process::Command::new(&exe_path).spawn() {
                ...
                Err(e) => { log::error!("Failed to spawn new terminal window: {}", e); }
            }
        } else { log::error!("Failed to get current executable path"); }
    }
    ...
    fn handle_reload_config(&mut self) {
        ...
        match Config::load() {
            ...
            None => { log::warn!("No config file found or failed to parse"); }
        }
    }
        ...
        for (tab_idx, tab) in self.tabs.iter_mut().enumerate() {
            ...
            if let Some(root) = &mut tab.root {
                ...
                root.for_each_leaf_mut(&mut |pane_id, terminal, child, scroll_offset, title| {
                    ...
                    loop {
                        match child.pty_mut().try_read(&mut buf) {
                            ...
                            Ok(n) => {
                                ...
                                if is_active_tab && pane_id == active_pane && !terminal.is_synchronized_output() {
                                    *needs_redraw = true;
                                }
                            }
                    ...
                    if received_output && *scroll_offset > 0 { *scroll_offset = 0; }
                    if terminal.take_title_changed() {
                        *title = terminal.title().to_string();
                        if is_active_tab && pane_id == active_pane {
                            if let Some(w) = window { w.set_title(title); }
                        }
                    }
                    if terminal.take_bell() { log::debug!("Bell!"); }
                    let responses = terminal.take_pending_responses();
                    for response in responses {
                        if let Err(e) = child.write_all(&response) { log::warn!("Failed to send response to PTY: {}", e); }
                    }
                });
            }
            if is_active_tab { tab.sync_title(); }
        }
    }
    ...
    fn render(&mut self) {
        ...
        for (pane_id, rect) in &layout {
            if let Some((terminal, _, scroll_offset)) = root.find_leaf_ref(*pane_id) {
                ...
                pane_infos.push(PaneRenderInfo { screen, selection, scroll_offset, rect: *rect, is_active });
            }
        }
        let tab_infos: Vec<TabInfo<'_>> = self.tabs.iter().map(|t| TabInfo { title: &t.title }).collect();
        if let Err(e) = renderer.render(&pane_infos, &dividers, self.tab_bar_height, &tab_infos, self.active_tab) {
    ...
    fn check_child(&mut self) -> bool {
        ...
        for (tab_idx, tab) in self.tabs.iter_mut().enumerate() {
            if let Some(root) = tab.root.take() {
                ...
                if dead_ids.is_empty() { tab.root = Some(root); continue; }
                let mut current = Some(root);
                for dead_id in &dead_ids {
                    if let Some(r) = current.take() { current = r.remove_pane(*dead_id); }
                }
                match current {
                    Some(remaining) => {
                    ...
                    None => { tabs_to_remove.push(tab_idx); }
                }
            } else { tabs_to_remove.push(tab_idx); }
        }
        for idx in tabs_to_remove.into_iter().rev() { self.tabs.remove(idx); }
