...
impl App {
    ...
    fn check_keybinding(&self, event: &winit::event::KeyEvent) -> Option<KeyAction> {
        ...
        match &event.logical_key {
            Key::Character(c) => {
                ...
                // Ctrl+Shift shortcuts (Linux/Windows style)
                if ctrl && shift {
                    match c.as_str() {
                ...
                // Ctrl shortcuts for zoom (Linux/Windows)
                #[cfg(not(target_os = "macos"))]
                if ctrl && !shift {
                    match c.as_str() {
                        "=" | "+" => return Some(KeyAction::ZoomIn),
    ...
    /// Handle copy (Ctrl+Shift+C or Cmd+C)
    ...
    fn handle_mouse_input(&mut self, button: MouseButton, state: ElementState) {
        ...
        if button == MouseButton::Left {
            if state == ElementState::Pressed {
                ...
                // Detect double/triple clicks (within 400ms and same position)
                let is_quick_click = elapsed < 400 && same_pos;
                ...
                // Start or update selection based on click count
